...
class SchedulerService:
    ...
    def _build_parameters(self, schedule: Schedule):
        ...
        if isinstance(schedule.parameters_json, list):
        ...
        if getattr(schedule, "runtime_parameters_json", None) is not None:
            parameters.append(
                {
                    "parameter_name": "runtime_parameters_json",
                    "parameter_value": json.dumps(schedule.runtime_parameters_json, ensure_ascii=False),
                    "is_secret": False,
                    "resolved_from_credential_item_id": None,
                }
            )
