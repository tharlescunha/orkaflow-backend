...
class TaskRepository:
    ...
    def get_child_task_for_parent_and_automation(
        self,
        parent_task_id: int,
        automation_id: int,
    ) -> Task | None:
    ...
    def get_open_task_for_automation(
        self,
        automation_id: int,
    ...
    def get_reserved_task_for_exclusive_groups(
        self,
        automation_id: int,
        exclude_task_id: int | None = None,
    ) -> Task | None:
        ...
        group_ids_stmt = (
            select(AutomationExclusiveGroupMember.group_id)
            .join(
                AutomationExclusiveGroup,
                AutomationExclusiveGroup.id == AutomationExclusiveGroupMember.group_id,
            )
            .where(AutomationExclusiveGroupMember.automation_id == automation_id)
            .where(AutomationExclusiveGroup.active == True)
        )
        group_ids = list(self.db.execute(group_ids_stmt).scalars().all())
        if not group_ids:
            return None
        ...
        stmt = (
            ...
            .join(
                AutomationExclusiveGroupMember,
                AutomationExclusiveGroupMember.automation_id == Task.automation_id,
            )
            .where(AutomationExclusiveGroupMember.group_id.in_(group_ids))
            .where(Task.status.in_(reserved_statuses))
    ...
    def has_active_lock_for_exclusive_groups(self, automation_id: int) -> bool:
        group_ids_stmt = (
            select(AutomationExclusiveGroupMember.group_id)
            .join(
                AutomationExclusiveGroup,
        ...
        lock_keys = [f"exclusive_group:{group_id}" for group_id in group_ids]
        stmt = (
            select(Lock.id)
            .where(Lock.lock_key.in_(lock_keys))
